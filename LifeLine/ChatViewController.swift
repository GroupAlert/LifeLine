...
class ChatViewController: UIViewController {
    ...
    override func viewDidLoad() {
        print ("this the valur of groupID: \(groupID)")
        print (PFUser.current()) // nill we need to get the current user 
        ...
        //        Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(self.autoMessage), userInfo: nil, repeats: true)
 ...
 //********************************
            chatMessage["user"] = PFUser.current()// line causing problems
    ...
    func autoSpeedMessage(speed: String){
        ...
        let chatMessage = PFObject(className: groupID) // className = group id
        chatMessage["text"] = "username" + "was going \(speed) "
        chatMessage["user"] = PFUser.current()
        chatMessage.saveInBackground()
    }
